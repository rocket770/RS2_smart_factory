...
class MainWindow(QMainWindow):
    ...
    def on_save_map_clicked(self):
        ...
        self.log(f"Save map requested with output prefix: {map_url or '<empty>'}")
        if not map_url:
            QMessageBox.warning(self, "Save Map", "Choose an output path prefix, for example /home/nick/maps/factory_merged")
            self.log("Save map blocked: empty output path prefix.", level="ERROR")
            return
        ...
        if not self.ros_node.save_map_client.wait_for_service(timeout_sec=1.0):
            ...
            self.log("Save map blocked: map_saver service is not available.", level="ERROR")
            return
        ...
        if not future.done():
            ...
            self.log("Save map request timed out waiting for map_saver response.", level="ERROR")
            return
        ...
        try:
            ...
            self.log(f"Failed to save map: {exc}", level="ERROR")
            self._log_traceback()
    ...
    def _request_explorer_return_home(self):
        ...
        try:
            ...
            self.log(f"Failed to request explorer return-home: {exc}", level="ERROR")
            self._log_traceback()
    ...
    def _nav_launch_command(self) -> List[str]:
        ...
        if mode_text == "Load Existing Map (AMCL)":
            ...
            if not os.path.exists(map_path):
                self.log(f"AMCL map path does not exist yet: {map_path}", level="ERROR")
            cmd.extend([
                "use_slam:=false",
                f"map:={map_path}",
            ])
    ...
    def on_start_nav_clicked(self):
        ...
        try:
            ...
            self.log(f"Nav stack launch blocked: {exc}", level="ERROR")
            return

        try:
            self._log_nav_launch_debug_inputs(cmd)
            if self.nav_mode_combo.currentText() == "Use SLAM + Explore":
            ...
            self.log(f"Nav stack process pid: {self.nav_process.pid}")
            if self.nav_mode_combo.currentText() == "Use SLAM + Explore":
            ...
            self.log(f"Failed to launch nav stack: {exc}", level="ERROR")
            self._log_traceback()
            self._clear_pending_explorer_start()

    def _log_nav_launch_debug_inputs(self, cmd: List[str]):
        mode_text = self.nav_mode_combo.currentText()
        self.log("Nav stack launch requested.")
        self.log(f"Nav mode: {mode_text}")
        self.log("Nav launch command: " + " ".join(cmd))
        self.log(f"Robot namespaces: {', '.join(self.ros_node.robot_namespaces) or '<none>'}")
        if mode_text == "Load Existing Map (AMCL)":
            map_path = self.map_path_input.text().strip()
            self.log(f"AMCL map YAML: {map_path or '<empty>'}")
            self.log(f"AMCL map YAML exists: {os.path.exists(map_path)}")
        else:
            save_path = self.save_map_path_input.text().strip()
            self.log(f"SLAM save-map prefix: {save_path or '<empty>'}")
            self.log(
                "Explorer services ready: "
                f"start={self.ros_node.explorer_start_client.service_is_ready()}, "
                f"stop={self.ros_node.explorer_stop_client.service_is_ready()}, "
                f"status={self.ros_node.explorer_status_client.service_is_ready()}, "
                f"return_home={self.ros_node.explorer_return_home_client.service_is_ready()}"
            )
    ...
    def _retry_pending_explorer_start(self):
        ...
        if self.nav_process is not None and self.nav_process.poll() is not None:
            self.log("Nav stack exited before explorer could be started.", level="ERROR")
        ...
        if now >= self._explorer_start_deadline:
            self.log("Timed out waiting for /multi_robot_explorer/start. Explorer remains paused.", level="ERROR")
        ...
        if future is not None:
            ...
            try:
                ...
                if resp.success:
                    ...
                    self.log(f"Explorer start returned failure: {resp.message}", level="ERROR")
                    self.nav_status_label.setText("SLAM launched, but explorer did not start cleanly.")
                self._clear_pending_explorer_start()
                return
            except Exception as exc:
                self.log(f"Explorer start call failed: {exc}", level="ERROR")
                self._log_traceback()
    ...
    def _explorer_has_robot_poses_for_home_capture(self) -> bool:
        ...
        try:
            ...
            self.log(f"Failed to read explorer status before start: {exc}", level="ERROR")
            self._log_traceback()
            return False
    ...
    def on_stop_nav_clicked(self):
        ...
        try:
            ...
            self.log(f"Failed to stop nav stack: {exc}", level="ERROR")
            self._log_traceback()
    ...
    def _on_explorer_control_response(self, action_name: str, future):
        try:
            ...
            self.log(f"Explorer {action_name} request failed: {exc}", level="ERROR")
            self._log_traceback()
            return
    ...
    def _refresh_nav_process_state(self):
        ...
        self.log(f"Nav stack exited with code {code}.", level="INFO" if code == 0 else "ERROR")
    ...
    def on_run_clicked(self):
        ...
        try:
            ...
            self.log(f"Failed to start MTSP solver: {exc}", level="ERROR")
            self._log_traceback()
