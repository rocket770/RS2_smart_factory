...
Saved maps are stored as a YAML metadata file plus an image file, usually PGM or PNG. The YAML records the image filename, resolution, origin, and occupancy thresholds; the PGM/PNG stores the occupancy grid pixels that AMCL and map server load.
...
![SLAM TF tree](images/slam_frames.png)

AMCL TF tree (`amcl_frames.png`):

![AMCL TF tree](images/amcl_frames.png)
...
| Costmap is out of bounds. | The robot is outside the loaded map, or the map has not loaded yet. | Explorer and Nav2 can reject goals because the target cell is outside the current costmap. | Confirm `/map` is publishing and view the robot pose in RViz relative to the map. | Wait for the map to load, relocalize or move the robot inside the mapped area, and confirm the correct map is loaded for the run. | ![Costmap out of bounds](images/costmap_out_of_bounds.png) |
