...
SLAM does not create the final shared map by itself. The per-robot maps are consumed by [Map Merger](Map-Merger), which publishes the shared `/map`. To write that merged map to disk, use [Map Saving](Map-Saving).
